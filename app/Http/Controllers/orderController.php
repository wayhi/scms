...
class orderController extends AppBaseController
{
    ...
    public function __construct(OrderRepository $orderRepo, goods_masterRepository $goods_masterRepo,PayableRepository $payableRepo, ReceivableRepository $receivableRepo)
    {
    ...
    public function index(Request $request)
    {
            ...
            $orders = $this->orderRepository->with(['customer','goods'])->paginate(10);
            $links = $orders->links();
    ...
    public function getInApproval(Request $request)
    {
        ...
        $orders = $this->orderRepository->with(['customer','goods'])->scopeQuery(function($query){
            return $query->where('process_status','1');
        })->paginate(10);
        $links = $orders->links();
        //->findByField('process_status',1);
        return view('orders.index')->with(['orders'=>$orders,'links'=>$links]);
    ...
    public function getInFunding(Request $request)
    {
        ...
        $orders = $this->orderRepository->with(['customer','goods'])->scopeQuery(function($query){
            return $query->where('process_status','2');
        })->paginate(10);
        $links = $orders->links();
        return view('orders.index')->with(['orders'=>$orders,'links'=>$links]);
    ...
    {
        $orders = $this->orderRepository->with(['customer','goods'])->scopeQuery(function($query){
            return $query->where('process_status','4');
        })->paginate(10);
        $links = $orders->links();
        return view('orders.index')->with(['orders'=>$orders,'links'=>$links]);
    ...
    {
       $orders = $this->orderRepository->with(['customer','goods'])->scopeQuery(function($query){
            return $query->where('process_status','6');
        })->paginate(10);
        $links = $orders->links();
        return view('orders.index')->with(['orders'=>$orders,'links'=>$links]);
    }
    ...
    public function getOverdue(Request $request)
    {
        ...
        $orders = $this->orderRepository->with(['customer','goods'])->scopeQuery(function($query){
            return $query->where(['process_status'=>3,'fund_status'=>5]); //
        })->paginate(10);
        $links = $orders->links();
        return view('orders.index')->with(['orders'=>$orders,'links'=>$links]);
    ...
    public function update($id, UpdateorderRequest $request)
    {
        ...
        switch ($request->action) {
            case '平台审核通过':
                ...
                //生成资金计划
                if($this->createPayables(1,$order,1) && $this->createReceivables(1,$order,1)){
                    Flash::success('订单：'.$order->order_number.' 已核准, 进入放款流程。');
                }else{
                    Flash::waring('订单：'.$order->order_number.' 已核准, 相关付款未能生成，请联系管理员。');
                }
            ...
            case '已放款':
                //生成资金计划
                if($this->createPayables(2,$order)){
                ...
                break;
            case '还款完成':
                $order = $this->orderRepository->update(['process_status'=>'6','fund_status'=>'8'], $id);
                Flash::success('订单：'.$order->order_number.' 已完成。');
                return redirect(route('oir'));
                break;
            case '订单取消':
                $order = $this->orderRepository->update(['process_status'=>'5','fund_status'=>'10'], $id);
                Flash::success('订单：'.$order->order_number.' 已取消。');
                return redirect(route('oir'));
                break;    
            case '拒绝':
                $order = $this->orderRepository->update(['process_status'=>'6','fund_status'=>'9'], $id);
